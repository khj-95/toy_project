...
public class AuthorizationFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if(uriArr.length != 0) {
			switch (uriArr[1]) {
				case "member":
					memberAuthorize(httpRequest, httpReponse, uriArr);
					break;
				case "admin":
					adminAuthorize(httpRequest, httpReponse, uriArr);
					break;
				case "board":
					boardAuthorize(httpRequest, httpReponse, uriArr);
					break;
				default:
					break;
			}
	...
	private void boardAuthorize(HttpServletRequest httpRequest, HttpServletResponse httpReponse, String[] uriArr) {
	...
	private void adminAuthorize(HttpServletRequest httpRequest, HttpServletResponse httpReponse, String[] uriArr) {
		...
		switch(uriArr[2]) {
		case "member" :
			//회원과 관련된 관리를 수행하는 admin의 등급을 AD01
			if(!adminGrade.DESC.equals("member")) {
				throw new HandlableException(ErrorCode.UNAUTHORIZED_PAGE);
			}
			break;
		case "board" :
			//게시판과 관련된 관리를 수행하는 admin의 등급을 AD02
	...
	private void memberAuthorize(HttpServletRequest httpRequest, HttpServletResponse httpReponse, String[] uriArr) {
